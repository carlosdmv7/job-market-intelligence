...
.PHONY: help install requirements fmt lint type test check ollama-pull ollama-up ollama-down ollama-status warehouse-init ingest ingest-all ingest-nl enrich sponsors-refresh dbt-deps dbt-build app clean
...
requirements: ## Regenerate requirements.txt for Streamlit Cloud (pip can't read the uv workspace)
	@{ \
	  echo "# Dependencies for the deployed Streamlit app (Streamlit Community Cloud)."; \
	  echo "#"; \
	  echo "# Streamlit Cloud installs with pip, which cannot read this repo's uv"; \
	  echo "# workspace ([tool.uv.sources]). So: third-party pins are exported from"; \
	  echo "# uv.lock (single source of truth, no drift), and the local packages are"; \
	  echo "# given as paths so pip never resolves 'jmi-core' from PyPI."; \
	  echo "#"; \
	  echo "# Regenerate with:  make requirements"; \
	  echo ""; \
	  uv export --package jmi-app --format requirements-txt --no-hashes \
	    --no-emit-workspace --no-dev 2>/dev/null \
	    | grep -v "^# This file was autogenerated" | grep -v "^#    uv export"; \
	  echo ""; \
	  echo "# --- local workspace packages (path installs) ---"; \
	  echo "./libs/jmi_core"; \
	  echo "./enrichment"; \
	  echo "./app"; \
	} > requirements.txt
